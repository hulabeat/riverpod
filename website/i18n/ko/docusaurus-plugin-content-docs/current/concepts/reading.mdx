...
#### [**DON'T**] `ref.read`를 `build` 메소드 안에서 **사용하지 마세요.**
...
그 목적은 칭찬할 만하지만 `ref.watch`를 대신 사용하여 똑같은 효과(빌드 수 감소)를 얻을 수 있다는 점에 유의하는 것이 중요합니다.

프로바이더들은 리빌드 횟수를 줄이면서 값을 얻을 수 있는 다양한 방법들을 제공합니다.
